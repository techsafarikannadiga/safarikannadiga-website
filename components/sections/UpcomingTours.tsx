...
export async function UpcomingTours() {
    ...
    return (
        <section className="section-padding bg-neutral-cream text-neutral-charcoal">
            <Container>
                <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="text-display text-neutral-charcoal mb-4">Upcoming Safari Tours</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {tours.map((tour) => (
                        <div key={tour.id} className={`bg-white rounded-card overflow-hidden shadow-card hover:shadow-xl transition-all duration-300 group ${tour.status === 'completed' ? 'opacity-80 grayscale hover:grayscale-0' : 'hover:-translate-y-1'}`}>
                            {/* Image */}
                            ...
                            <div className="p-6">
                                {/* Dates */}
                                <div className={`flex items-center gap-2 font-bold mb-3 ${tour.status === 'completed' ? 'text-neutral-gray' : 'text-safari-gold'}`}>
                                ...
                                <h3 className="text-xl font-bold text-neutral-charcoal mb-1 font-heading">
                                    {tour.title}
                                </h3>
                                <p className="text-neutral-gray text-sm mb-4">{tour.destination}</p>

                                {/* Highlights */}
                                <ul className="space-y-2 mb-6">
                                    {tour.highlights.map((highlight, i) => (
                                        <li key={i} className="flex items-center gap-2 text-sm text-neutral-gray">
                                            <svg className={`w-4 h-4 shrink-0 ${tour.status === 'completed' ? 'text-neutral-gray' : 'text-safari-gold'}`} fill="currentColor" viewBox="0 0 20 20">
                                ...
                                {tour.status === 'completed' || tour.status === 'sold-out' ? (
                                    <div className="w-full bg-neutral-cream text-neutral-gray py-3 rounded-full font-bold text-center block cursor-not-allowed">
                                    ...
                                    <a
                                        ...
                                        className="w-full bg-safari-gold text-white py-3 rounded-full font-bold text-center flex items-center justify-center gap-2 hover:bg-safari-gold-dark transition-colors shadow-md hover:shadow-lg"
                                    >
                ...
                <div className="mt-12 text-center">
                    ...
                    <Link href="/contact" className="inline-flex items-center gap-2 border-2 border-forest-green text-forest-green px-8 py-3 rounded-full font-bold hover:bg-forest-green hover:text-white transition-colors">
                        Request Custom Tour
                    </Link>
                </div>
            </Container>
